...
def generation_token(username, password_hash, password, is_refresh=True):
    if username is None:
        return None
    ...
    # 15 min token for access_token
    min15 = datetime.datetime.utcnow() + datetime.timedelta(minutes=current_app.config['TOKEN_EXPIRE_MINUTE'])
    data["exp"] = calendar.timegm(min15.timetuple())
    access_token = jwt.encode(data, key=current_app.config['SECRET_KEY'],
                              algorithm=current_app.config['ALGORITHM'])

    # day token for access_token
...
def approve_token(token):
    ...
    return generation_token(username=username, password=password, password_hash=None, is_refresh=True)
